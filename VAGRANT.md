...
- `http://localhost:55080` mostra uma lista obtida por `SELECT`;
- `http://localhost:55080/phpmyadmin` permite inspecionar o banco;
- FTP responde em `localhost:55021` para testes locais;
...
Abra `http://localhost:55080` no navegador. A página deve exibir `Caderno de projetos`, `Curso de PHP` e `Laboratorio DevOps`.
...
```text
http://localhost:55080/phpmyadmin
```
...
O serviço usa vsftpd, bloqueia acesso anônimo e encaminha a porta 21 do guest para a porta 55021 do Windows:
